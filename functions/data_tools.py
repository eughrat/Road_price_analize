...
def get_element(df, word:str, col_to_group='Elementy_rozliczeniowe', 
                    col_groupby= 'Droga', 
                    agg_dict={'Ilosc': 'sum','Dlugosc_drogi':'max'}):
...
def get_multiple_elem_method_or(df, *args, 
                    col_to_group='Elementy_rozliczeniowe', col_groupby= 'Droga',
                    agg_dict={'Ilosc': 'sum','Dlugosc_drogi':'max'}):
    road_elem_df = df[df[col_to_group].str.contains('|'.join(list(args)))]
    road_elem_df = road_elem_df.groupby(col_groupby).agg(agg_dict)
    return road_elem_df
