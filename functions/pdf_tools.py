...
def read_pdfs(path:str,new_col_names =['Lp','CPV','Numer_Specyfikacji_Technicznej','Elementy_rozliczeniowe','Jednostka','Ilosc','Cena_jedn','Wartosc_calkowita','Droga','Rok','Kategoria']):
...
def clean_pdf(df,words_to_replace={'\n': '','': np.nan},col_to_drop=['Lp','CPV','Numer_Specyfikacji_Technicznej']):
...
def match_category(row, cor_cat_list=['ROBOTY PRZYGOTOWAWCZE','OZNAKOWANIA DROG','ZIELEN DROGOWA','PODBUDOWY','NAWIERZCHNIE','ROBOTY WYKONCZENIOWE','ELEMENTY ULIC','WARUNKI OGOLNE','ROBOTY ZIEMNE','ODWODNIENIE','ODWODNIENIE KORPUSU DROGOWEGO','INNE ROBOTY']):
